...
def _get_atol_rtol(name, b_norm, tol=_NoValue, atol=0.0, rtol=1e-5):
    """
    A helper function to handle tolerance deprecations and normalization
    """
    if tol is not _NoValue:
        msg = (
            f"'scipy.sparse.linalg.{name}' keyword argument `tol` is "
    ...
    if atol == "legacy":
        msg = (
            f"'scipy.sparse.linalg.{name}' called with `atol='legacy'`. "
    ...
    if atol is None:
        msg = (
            f"'scipy.sparse.linalg.{name}' called without specifying "
...
def gmres(
    ...
    *,
    ...
    if restrt is not _NoValue:
        msg = (
            "'gmres' keyword argument 'restrt' is deprecated "
