...
def test_support_alternative_backends(xp, data, f_name_n_args):
    ...
    elements = dict(
        min_value=dtype_np(-10), max_value=dtype_np(10), allow_subnormal=False
    )
    args_np = [
        np.asarray(data.draw(npst.arrays(dtype_np, shape, elements=elements)))
        for shape in shapes
    ]
    ...
    xp_assert_close(
        xp.sqrt(xp.mean(res**2)),
        xp.sqrt(xp.mean(ref**2)),
        rtol=eps**0.5,
        atol=eps * 10,
