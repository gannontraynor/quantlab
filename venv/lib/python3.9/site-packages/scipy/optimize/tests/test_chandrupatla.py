...
class TestChandrupatlaMinimize:
    ...
    def test_flags(self):
        ...
        res = _chandrupatla_minimize(
            f, [0] * 4, [2] * 4, [np.pi] * 4, args=args, maxiter=10
        )
    ...
    def test_nit_expected(self, case):
        ...
        res = _chandrupatla_minimize(
            func, x1, x2, x3, xatol=xatol, fatol=fatol, xrtol=xrtol, frtol=frtol
        )
    ...
    def test_input_validation(self):
        ...
        with pytest.raises(ValueError, match=message):
            _chandrupatla_minimize(
                lambda x: [x[0], x[1], x[1]], [-3, -3], [0, 0], [5, 5]
            )
...
class TestChandrupatla(TestScalarRootFinders):
    ...
    def test_vectorization(self, shape):
        ...
        assert np.all(
            np.abs(res.fun[finite])
            == np.minimum(np.abs(res.fl[finite]), np.abs(res.fr[finite]))
        )
    ...
    def test_dtype(self, root, dtype):
        ...
        res = _chandrupatla_root(f, dtype(-3), dtype(5), args=(root,), xatol=1e-3)
        assert res.x.dtype == dtype
