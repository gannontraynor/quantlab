    ...
    *,
    ...
    with np.errstate(over="ignore", invalid="ignore", divide="ignore"):
        ...
        temp = eim._initialize(
            f, (c,), args, complex_ok=True, preserve_shape=preserve_shape
        )
    ...
    work = _RichResult(
        ...
        eps=eps,
        a=a.reshape(-1, 1),
        b=b.reshape(-1, 1),  # integration limits
        n=minlevel,
        nit=nit,
        nfev=nfev,
    ...
    def pre_func_eval(work):
        ...
        work.h = h0 / 2**work.n
        xjc, wj = _get_pairs(
            work.n, h0, dtype=work.dtype, inclusive=(work.n == minlevel)
        )
    ...
    def post_func_eval(x, fj, work):
        ...
        if work.log:
            fj[work.abinf] += np.log(1 + work.xj[work.abinf] ** 2) - 2 * np.log(
        ...
        if work.Sk.shape[-1]:
            Snm1 = work.Sk[:, -1]
            Sn = (
                special.logsumexp([Snm1 - np.log(2), Sn], axis=0)
                if log
                else Snm1 / 2 + Sn
            )
    ...
    def check_termination(work):
        ...
        if work.nit == 0:
            ...
            i = (
                (work.rerr < rtol) | (work.rerr + np.real(work.Sn) < atol)
                if log
...
def _euler_maclaurin_sum(fj, work):
    ...
    Sn = (
        special.logsumexp(fjwj + np.log(work.h), axis=-1)
        if work.log
...
def _estimate_error(work):
    ...
    if work.Sk.shape[-1] == 0:
        ...
        Snm1 = (
            special.logsumexp(fjwj, **axis_kwargs) + np.log(h)
            if work.log
...
def _nsum(
    f, a, b, step=1, args=(), log=False, maxterms=int(2**20), atol=None, rtol=None
):
...
def _integral_bound(f, a, b, step, args, constants):
    ...
    left, left_error, left_nfev = _direct(
        f, a, k, step, args, constants, inclusive=False
    )
