...
@skip_if_array_api(
    "torch", reasons=["torch.fft not yet implemented by array-api-compat"]
)
class TestFFTShift:
...
class TestFFTFreq:
    ...
    def test_definition(self, xp):
        ...
        try:
            x = xp.asarray(
                [0, 1, 2, 3, 4, -4, -3, -2, -1], dtype=xp.float64, device=device
            )
            x2 = xp.asarray(
                [0, 1, 2, 3, 4, -5, -4, -3, -2, -1], dtype=xp.float64, device=device
            )
        except TypeError:
